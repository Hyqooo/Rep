...
/*
	Функция, проверяющая цифра введена или нет.
	Если цифра вернется 1, если нет, то 0
	
	Замечание: при подобном построении, когда есть какое-то условие, при котором
	возвращаются разные значения, должны быть обязательно прописаны, возвращаемые значения
	при всех путях выполнения (попробуй убрать весь блок else)
*/
